...
void loop() {
  ...
  if (alarmState != 2) alarmState = CircuitPlayground.slideSwitch();
...
void displayHour(uint8_t* p) {
  if (*p & (1<<0)) CircuitPlayground.setPixelColor(0, colorh);
  if (*p & (1<<1)) CircuitPlayground.setPixelColor(1, colorh);
  if (*p & (1<<2)) CircuitPlayground.setPixelColor(2, colorh);
  if (*p & (1<<3)) CircuitPlayground.setPixelColor(3, colorh);
  if (*p & (1<<4)) CircuitPlayground.setPixelColor(4, colorh);
}

void displayMinute(uint8_t* p) {
  CircuitPlayground.redLED(*p & (1<<0));
  if (*p & (1<<1)) CircuitPlayground.setPixelColor(9, colorm);
  if (*p & (1<<2)) CircuitPlayground.setPixelColor(8, colorm);
  if (*p & (1<<3)) CircuitPlayground.setPixelColor(7, colorm);
  if (*p & (1<<4)) CircuitPlayground.setPixelColor(6, colorm);
  if (*p & (1<<5)) CircuitPlayground.setPixelColor(5, colorm);
}
...
void alarm() {
  ...
  while(CircuitPlayground.leftButton() && (millis() - aStart) < 60000 && CircuitPlayground.slideSwitch()); // need time check here too in case time runs out
}
